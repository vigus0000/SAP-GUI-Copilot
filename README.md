...
- `SAP_BUSINESS_TOOLS_ENABLED=true` 時，主專案會從 `sap_business_tools.py` 註冊流程級 MCP tools，並自動加入 core allowlist 與 Auto prompt。內建支援 `sap_get_order_overview`（VA03 銷售訂單全貌）與 `sap_analyze_delivery_block`（VA03/VKM1/MD04/FBL5N 訂單卡關診斷）。新增同類工具時優先在 `sap_business_tools.py` 補 `BusinessToolSpec`，實作仍放在 `external/mcp-sap-gui`。
